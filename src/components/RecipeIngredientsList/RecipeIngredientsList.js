...
const RecipeIngredientsList = ({ ingredients }) => {
  return ingredients.map((recipeIngredient, key) => {
    ...
    const ingredientName = recipeIngredient.unitOfMeasurement?.length ? 
        recipeIngredient.name : pluralize(recipeIngredient.name, Number(recipeIngredient.quantity));

    const ingredientLabel = `${recipeIngredient.quantity}${recipeIngredient.unitOfMeasurement || ''} ${ingredientName}`;
